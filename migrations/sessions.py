...
def parse_date(val):
    ...
    # Handle raw serial number if present
    try:
...
# 1. Extract and migrate unique On-Site Managers
managers_data = {}
for _, row in df.iterrows():
    m_name = clean_str(row["manager"])
    m_mobile = clean_str(row["manager-mobile"])
...
# 2. Map and migrate Sessions
sessions_records = []
for _, row in df.iterrows():
    school_name = clean_str(row["school"])
    ...
    caterer_name = clean_str(row["caterer"])
    caterer_id = caterer_name_to_id.get(caterer_name)
    
    manager_name = clean_str(row["manager"])
    manager_id = manager_name_to_id.get(manager_name)

    session_date = parse_date(row["date"])
    if not session_date:
    ...
    rec = {
        ...
        "Date": session_date,
        "Day": clean_str(row["day"]),
        "On-Site Manager": [manager_id] if manager_id else None,
        "Start Time": clean_str(row["start-time"]),
        "End Time": clean_str(row["end-time"]),
        "Dinner Time": clean_str(row["dinner-time"]),
        "Year Levels": clean_str(row["year-levels"]),
        "Building": clean_str(row["Building"])
    }
    sessions_records.append(rec)
