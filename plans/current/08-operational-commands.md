...
AIRTABLE_ID=<base-id>
URL_ORIGIN=http://<server-ip>:<port>   # needed by send_meals_links / send_qr_emails / evaluate_caterers
# Optional — enables LLM extraction during migration
CLAUDE_CODE_API_KEY=<...>   # or ANTHROPIC_API_KEY=<...>

# Install deps (Arch users: --user --break-system-packages or system pkgs)
...
./run orders generate              # write Orders + Weekly Orders
...
./run forms qr --first                                # appends &first=1 (hides rating)

# Email QR codes to on-site managers (uses URL_ORIGIN from .env)
./run forms qr send
./run forms qr send --dry-run
./run forms qr send --immediate
...
./run forms send parents --first       # start-of-term mode (hides rating card)
```

## Caterer management

```bash
./run caterer evaluate                 # rolling-rating check, queue switch proposals
./run caterer evaluate --dry-run       # print proposals without writing to Airtable
./run caterer evaluate --immediate     # flag alert emails for immediate send

# Force-create a proposal (bypasses rating thresholds)
python scripts/actions/evaluate_caterers.py --force \
       --session "MacGregor State High School - Thursday" \
       [--incoming <caterer_id>] [--dry-run]

./run caterer switch <proposal_id>     # execute an Approved proposal
./run caterer switch <proposal_id> --approve   # also accept Pending
./run caterer switch <proposal_id> --dry-run
```

Approval / rejection of a proposal also has a webapp UI at
`/switch-proposal.html?id=<rec_id>`; the email generated by
`evaluate_caterers.py` links to that page directly.

## Webapp

```bash
./run host                       # serve webapp/ + data/ on 0.0.0.0:8000 (default)
./run host --port 9000           # alternate port
```

Prints both `http://localhost:<port>/index.html` *suggested-link template*
and a LAN URL. The actual entry path is `/meals.html?session=…`.

## Scripts (free-form)

```bash
./run script <name>                  # python scripts/actions/<name>.py
./run script clear_database --yes    # WIPES the entire base — confirmation prompt unless --yes
./run script send_qr_emails --dry-run
```
...
> The standalone `verify_migration.py` is retired and lives under `old/`.
...
| Email QR codes | `./run forms qr send` |
| Email preference links | `./run forms send {parents\|students}` |
| Run any actions/ script | `./run script <name>` |
| Wipe DB | `./run script clear_database --yes` |
