...
class TestIsStudentExcluded(unittest.TestCase):
    ...
    def test_excluded_matching_year_level(self):
        ...
        exclusion = Record(id="exc002", fields={
            "School": [fixtures.SCHOOL_A_ID],
            "Date":   "2026-02-02",
            "Affected Year Levels": ["10", "11"],
        })
        idx = self._index_with_exclusions([exclusion])
        stu  = fixtures.student_normal().fields  # Year Level = 10
        sess = fixtures.session_monday().fields
        self.assertTrue(is_student_excluded(stu, sess, date(2026, 2, 2), idx))
    ...
    def test_not_excluded_wrong_year_level(self):
        ...
        exclusion = Record(id="exc003", fields={
            "School": [fixtures.SCHOOL_A_ID],
            "Date":   "2026-02-02",
            "Affected Year Levels": ["12"],
        })
        idx = self._index_with_exclusions([exclusion])
        stu  = fixtures.student_normal().fields  # Year Level = 10, not 12
        sess = fixtures.session_monday().fields
        self.assertFalse(is_student_excluded(stu, sess, date(2026, 2, 2), idx))
...
class TestAssignFallbackMeal(unittest.TestCase):
    ...
    def test_popularity_weighting_follows_counts(self):
        ...
        item_a = Record(id="iA", fields={"Menu Item Name": "Item A", "Caterer": ["c"], "Dietary Tags": []})
        item_b = Record(id="iB", fields={"Menu Item Name": "Item B", "Caterer": ["c"], "Dietary Tags": []})
        counts = {"iA": 100, "iB": 0}
...
class TestAssignVarietyMeal(unittest.TestCase):
    ...
    def test_max_items_cap_prevents_new_item(self):
        ...
        item_a = Record(id="iA", fields={"Menu Item Name": "Item A", "Caterer": ["c"], "Dietary Tags": []})
        item_b = Record(id="iB", fields={"Menu Item Name": "Item B", "Caterer": ["c"], "Dietary Tags": []})
        item_c = Record(id="iC", fields={"Menu Item Name": "Item C", "Caterer": ["c"], "Dietary Tags": []})
    ...
    def test_dietary_exception_allows_new_item_despite_cap(self):
        ...
        item_a = Record(id="iA", fields={"Menu Item Name": "Chicken Fried Rice", "Caterer": ["c"], "Dietary Tags": []})
        item_b = Record(id="iB", fields={"Menu Item Name": "Beef Burger",        "Caterer": ["c"], "Dietary Tags": []})
        item_c = Record(id="iC", fields={"Menu Item Name": "Vegan Bowl",          "Caterer": ["c"], "Dietary Tags": [fixtures.DIET_VEGAN_ID]})
...
class TestComputeMaxVariety(unittest.TestCase):
    ...
    def test_returns_4_when_students_satisfy_constraint(self):
        # "Min Qty 4 Items": 3, 12 students → 12 >= 4*3 → 4 items
    ...
    def test_falls_back_when_students_insufficient(self):
        # "Min Qty 4 Items": 3, 10 students → 10 < 4*3=12 → fallback = max(1, 10//3) = 3
        cf = {"Caterer Name": "X", "Min Qty 4 Items": 3}
    ...
    def test_prefers_higher_n_when_both_feasible(self):
        # 30 students, Min Qty 5 Items=5, Min Qty 4 Items=3 → 30>=5*5=25 → 5 items
        cf = {"Min Qty 5 Items": 5, "Min Qty 4 Items": 3}
...
class TestEnforceMinQty(unittest.TestCase):
    ...
    def _make_index_for_students(self, students: list[Record]) -> OrderingIndex:
        items = [
            Record(id="iA", fields={"Menu Item Name": "Item A", "Caterer": [fixtures.CATERER_A_ID], "Dietary Tags": []}),
            Record(id="iB", fields={"Menu Item Name": "Item B", "Caterer": [fixtures.CATERER_A_ID], "Dietary Tags": []}),
            Record(id="iC", fields={"Menu Item Name": "Item C", "Caterer": [fixtures.CATERER_A_ID], "Dietary Tags": []}),
            Record(id="iD", fields={"Menu Item Name": "Item D", "Caterer": [fixtures.CATERER_A_ID], "Dietary Tags": []}),
        ]
    ...
    def test_no_violations_unchanged(self):
        ...
        cf       = {"Caterer Name": "X", "Min Qty 4 Items": 2}
...
class TestFlipIncomingCaterers(unittest.TestCase):
    ...
    def test_no_sessions_with_incoming_caterer(self):
        db = MockDatabase()
        db.Sessions._records = [
            Record(id="s1", fields={"Caterer": ["cOld"]}),
        ]
        flip_incoming_caterers(db, dry_run=False)
        self.assertEqual(db.Sessions.updates, [])
...
class TestRegisterOrdersPipeline(unittest.TestCase):
    ...
    def test_assigns_meals_for_eligible_students(self):
        ...
        self.assertEqual(wo["Total Meals"], 2)
        self.assertEqual(wo["Caterer"], [fixtures.CATERER_A_ID])
    ...
    def test_explicit_preference_respected(self):
        ...
        assigned_items = [o["Menu Item"][0] for o in db.Orders.created_fields]
